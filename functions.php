...
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-  
if (!function_exists('nominee_admin_scripts')) :
    function nominee_admin_scripts() {
        wp_enqueue_style('nominee-admin-style', get_template_directory_uri() . '/css/admin-style.css', array(), NULL);
    }
endif;
add_action( 'admin_enqueue_scripts', 'nominee_admin_scripts', 999 );
